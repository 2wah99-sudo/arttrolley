...
// ARTTROLLEY accent (terracotta #c1502e) used for the scan sweep and dot mask
const ACCENT = { r: 0.76, g: 0.31, b: 0.18 };
...
const PostProcessing = ({
  ...
  const render = useMemo(() => {
    ...
    const redOverlay = vec3(ACCENT.r, ACCENT.g, ACCENT.b).mul(oneMinus(scanLine)).mul(0.4);
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const mask = dot.mul(flow).mul(vec3(ACCENT.r * 13, ACCENT.g * 13, ACCENT.b * 13));
...
export const Html = ({
  ...
  subtitle = 'Handblock sarees & kurtis from the karigars of Rajasthan.',
